...
import { Sparkles, ArrowRight, Copy, CheckCircle2, RefreshCw, Type, Settings2 } from 'lucide-react';
...
export default function AIHumanizer() {
  ...
  const [mode, setMode] = useState<'precise' | 'balanced' | 'creative'>('precise');
  ...
  const handleHumanize = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8000/api/humanize', {
        ...
        body: JSON.stringify({
          text: inputText,
          mode: 'precise', // Forced to precise as requested
          length_ratio: 0.15,
        ...
        <div className="flex-1 bg-white rounded-xl border border-gray-300 shadow-sm overflow-hidden min-h-[500px] flex flex-col relative">
          {!outputText && !isProcessing && (
            <div className="absolute inset-0 flex flex-col items-center justify-center text-gray-400 p-8 text-center bg-gray-50">
              ...
              <p className="text-sm mt-2 max-w-xs">We'll rewrite your text to sound more natural, empathetic, and professional.</p>
            </div>
          )}
          ...
          {isProcessing && (
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-white/90 z-10">
              ...
              <p className="text-xs text-gray-400 mt-2">Mode: {mode.toUpperCase()}</p>
            </div>
          )}
